...
export default {
  ...
  methods: {
    ...
    changeCabOption(value, key) {
      ...
      // console.log('this.cabOption.selectedFields', this.cabOption.selectedFields);
      // console.log('this.cabsOptions', this.cabsOptions);
      // console.log('Object.keys', Object.keys(this.cabsOptions).length);

      // console.log('this.selectedCabs-5-5', this.selectedCabs);
      // console.log('this.cabOption-5-5', this.cabOption);
      if (this.cabOption.selectedFields.length > value.length){
        console.log('LLLLLLLLLLLLLLLLLLL - 1111111');
        differenceValue =  this.cabOption.selectedFields.find(item => !value.includes(item));
      } else {
        console.log('KKKKKKKKKKKKKKK ---- 22222');
        differenceValue =  value.find(item => !this.cabOption.selectedFields.includes(item));
      }

      console.log('differenceValue11', differenceValue);
      if (this.selectedCabs.length === 1) {
        console.log('BBBBBB11111');
        this.updateInputFieldStatus(false, differenceValue);
      }
      ...
      if (this.selectedCabs.length > 1) {
        ...
        for (const id in this.cabsOptions) {
          ...
          const includesValue = selectedFields.includes(differenceValue);
          console.log('includesValue', includesValue);
          console.log('overallIncludesValue', overallIncludesValue);
          // let differenceValue1 =  this.cabOption.selectedFields.find(item => !value.includes(item));
          // console.log('differenceValue111', differenceValue1);

          if (overallIncludesValue === null) {
            overallIncludesValue = includesValue;
          } else if (overallIncludesValue !== includesValue) {
            overallIncludesValue = 'mixed';
            break;
          }
        }
        if (overallIncludesValue !== 'mixed'){
          this.updateInputFieldStatus(overallIncludesValue, differenceValue);
          console.log();
        }
        console.log('Overall Includes Value:111', overallIncludesValue);
      }
    },
