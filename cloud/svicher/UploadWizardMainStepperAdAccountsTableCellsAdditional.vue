  ...
  watch: {
    'selectedCabs': {
      async handler(newVal ) {
        this.updateSelectedCabinet1 = [...newVal];
      }
    }
  },
    ...
    changeCabOption(value, key) {
      ...
      if (key === 'selectedFields'){
        if (this.cabOption.selectedFields.length > value.length){
          selectedDifferenceValue =  this.cabOption.selectedFields.find(item => !value.includes(item));
        } else {
          selectedDifferenceValue =  value.find(item => !this.cabOption.selectedFields.includes(item));
        }
      }
      ...
      console.log('selectedDifferenceValue', selectedDifferenceValue);
      // if (this.selectedCabs.length === 1) {
      //   this.updateInputFieldStatus(false, selectedDifferenceValue);
      // }
      console.log('this.cabsOptions-5-5-5', this.cabsOptions);
      // if (this.selectedCabs.length > 1) {
      let overallIncludesValue = null;
      // console.log('this.cabsOptions', this.cabsOptions);
      for (const id in this.cabsOptions) {
        if (!this.updateSelectedCabinet1.some(item => item.id === id)) {

          continue;
        }
        console.log('this.updateSelectedCabinet-3-3', this.updateSelectedCabinet1);
        let selectedFields = [...this.cabsOptions[id].selectedFields];
        const includesValue = selectedFields.includes(selectedDifferenceValue);

        if (overallIncludesValue === null) {
          overallIncludesValue = includesValue;
        } else if (overallIncludesValue !== includesValue) {
          overallIncludesValue = 'IsNotSame';
          break;
        }
      }

      if (overallIncludesValue !== 'IsNotSame'){
        this.updateInputFieldStatus(overallIncludesValue, selectedDifferenceValue);
      }
      // }
    },
