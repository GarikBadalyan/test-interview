...
export default {
  ...
  created () {
    console.log('AAAAAAAAAAAAAAAAAAa223344')
  ...
  methods: {
    ...
    toggleItem (item, event) {
      if (item.id === 'all') {
        this.$nextTick(() => {
          ...
          if (parentDiv) {
            // parentDiv.style.backgroundColor = this.isSelected(item) ? 'rgba(33, 150, 243, 0.2)' : 'rgba(33, 33, 33, 0.46)'
            parentDiv.style.backgroundColor = this.isSelected(item) && this.$vuetify.theme.dark
                ? 'rgba(33, 150, 243, 0.2)' : this.isSelected(item) && !this.$vuetify.theme.dark
                    ? 'rgba(33, 150, 243, 0.1)' : 'initial'
            parentDiv.style.color = this.isSelected(item) ? 'rgb(33, 150, 243)'
                : this.$vuetify.theme.dark ? '#FFFFFF' : 'rgba(0, 0, 0, 0.87)'
          }
        })
    ...
    filterByUsers (users) {
      ...
      if (!menu.isActive || users.length === 0) {
        // Es depq@ asxatum e erb vor krestiki vra enq sexmum
        const firstDiv = document.querySelector('.v-menu__content div:first-child')
        console.log('firstDiv', firstDiv)
        if (firstDiv) {
          const anotherChildDiv = firstDiv.querySelector('div') // Replace 'div' with your specific selector
          console.log('anotherChildDiv111', anotherChildDiv)
          anotherChildDiv.style.backgroundColor = this.$vuetify.theme.dark
              ? 'rgba(33, 33, 33, 0.46)' : !this.$vuetify.theme.dark
                  ? '#FFFFFF' : 'initial'
          anotherChildDiv.style.color = this.$vuetify.theme.dark ? '#FFFFFF' : 'rgba(0, 0, 0, 0.87)'
        }
      }

      if (users.includes('all')) {
        this.$nextTick(() => {
          console.log('users.length111', users.length)
          console.log('this.computedUsers.length222', this.computedUsers.length)
          const menu = this.$refs.autocomplete.$refs.menu
          menu.isActive = !menu.isActive
          const selectedItem = document.querySelector('.selected-item-list div:first-child')
          console.log('selectedItem', selectedItem)

          const firstDiv = document.querySelector('.v-menu__content div:first-child')
          console.log('firstDiv555', firstDiv)
          if (firstDiv && users[0] === 'all') {
            const anotherChildDiv = firstDiv.querySelector('div')
            console.log('anotherChildDiv444', anotherChildDiv)
            console.log('users999', users)
            anotherChildDiv.style.backgroundColor = this.$vuetify.theme.dark
                ? 'rgba(33, 150, 243, 0.2)' : !this.$vuetify.theme.dark
                    ? 'rgba(33, 150, 243, 0.1)' : 'initial'
            anotherChildDiv.style.color = 'rgba(33, 150, 243, 0.7)'
          } else if (firstDiv && users.length > 1 && users[0] !== 'all' && users.length === this.computedUsers.length) {
            const anotherChildDiv = firstDiv.querySelector('div')
            console.log('anotherChildDiv8-8-8-8-8', anotherChildDiv)
            console.log('this.computedUsers', this.computedUsers)
            console.log('users', users)
            anotherChildDiv.classList.add('aaa')
        ...
        console.log('YYYYYYYYYYYYYYYXXXXXXXXXX')
        this.filtersUsers = ''
