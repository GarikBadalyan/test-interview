  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        if (firstMenuItem && this.computedUsers.length - 1 === this.filters.users.length) {
          if (this.$vuetify.theme.dark) {
            ...
            this.applyStylesToChildDiv(firstMenuItem, 'rgba(33, 150, 243, 0.12)', '#1976d2')
          }
        }
      })
    },
    ...
    toggleItem (item, event) {
      if (item.id === 'all') {
        this.$nextTick(() => {
          ...
          const parentDiv = event.currentTarget.closest('.v-list-item div:first-child')
    ...
    updateStylesForThemeV2 (firstDiv, isDarkTheme) {
      ...
      if (isDarkTheme) {
        ...
        this.applyStylesToChildDiv(anotherChildDiv, 'rgba(33, 150, 243, 0.1)', '#1976d2')
      }
    },
