          ...
          <div v-if="profile && profile.role && ['admin', 'teamlead'].includes(profile.role)">
            ...
            >
              <template #[`item`]="{item}">
                ...
                <!--                  <span v-if="item.id === profile.id">({{ $t('common.you').toLowerCase() }})</span>-->
                <!--                </span>-->
                <!--                &lt;!&ndash; почта, если есть отображаемое имя &ndash;&gt;-->
                <!--                <span-->
                <!--                  v-if="item.displayName"-->
                <!--                  style="font-size: 12px; margin-left: 8px; color: grey; padding-top: 4px;"-->
                <!--                >-->
...
export default {
  ...
  created () {
    ...
    console.log(this.filters.users, 'gggggg')
  },
  ...
  computed: {
    ...
    computedUsers () {
      ...
      if (this.users.all.length && this.profile.team.isOwner) {
        return [{ username: 'Select all', id: 'all' }, ...users]
  ...
  methods: {
    ...
    toggleItem (item) {
      console.log(this.filters.users)
      debugger
      if (item.id === 'all') {
    ...
    filterByUsers (users) {
      // console.log('sssssssssssssssssss', users)
      // this.filtersUsers = ''
