...
export default {
  ...
  methods: {
    ...
    isSelected (item) {
      console.log('11111aaaaa', this.filters.users)
    ...
    toggleItem (item) {
      ...
      if (item.id === 'all') {
        console.log(JSON.parse(JSON.stringify(this.computedUsers)), 'computedUsers')
        console.log(JSON.parse(JSON.stringify(this.filters.users)), 'this.filters.users')
        console.log(JSON.parse(JSON.stringify(item)), 'item')

        if (this.computedUsers.length !== this.filters.users.length) {
          console.log('if')
          this.computedUsers.forEach(el => {
            if (!this.filters.users.includes(el.id)) {
