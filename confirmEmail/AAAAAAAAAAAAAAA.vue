...
export default {
  ...
  watch: {
    ...
    'cabOption.pixel': function(newValue) {
      // Ensure that the watcher is not causing issues
      if (newValue){
        console.log('watch');
      }
      console.log('cabOption.pixel watcher2222222:', newValue);
    },
  ...
  created() {
    ...
    if (selectedFp) {
      ...
      this.totalFp = this.fpList.length;
    }
  ...
  methods: {
    // shouldShowCopyIcon(value) {
    //   if (Object.keys(value).length !== 0) {
    //     console.log('value value', value);
    //     console.log('value valueselectedPixel.name', value.selectedPixel.name);
    //     return value.selectedPixel.name !== null && value.selectedPixel.name !== undefined && value.selectedPixel.name !== '';
    //   }
    //
    //   // Check if the value is not empty (you can adjust the condition based on your data structure)
    ...
    async onIntersectFpList() {
      ...
      if (this.fpList.length < this.totalFp) {
        this.currentPageForFp += 1;
