        ...
        >
          ...
          <div v-if="profile && profile.role && ['admin', 'teamlead'].includes(profile.role)">
            ...
            >
              ...
              <!--                <v-chip close>-->
              <!--                  <span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis">{{ item.username }}</span>-->
              <!--                </v-chip>-->
              <!--              </template>-->
            </v-autocomplete>
          </div>
...
export default {
  ...
  computed: {
    ...
    computedUsers () {
      ...
      console.log('this.profile.team.isOwner', this.profile.team.isOwner)
      if (this.users.all.length && this.profile.team.isOwner) {
        users.unshift({ username: 'Select all', id: 'all' })
        // users.unshift({ username: this.$t('common.selectAllUser'), id: 'all' })
      }
  ...
  methods: {
    ...
    isSelected (item) {
      ...
      // const index = this.filters.users.indexOf('all')
      //
      // if (index !== -1) {
    ...
    toggleItem (item) {
      console.log('item222', JSON.parse(JSON.stringify(item)))
    ...
    //     console.log('this.computedUsers', JSON.parse(JSON.stringify(this.computedUsers)))
    //     this.computedUsers.forEach(el => {
    //       if (!this.filters.users.includes(el.id) && el.id !== 'all') {
    //         this.filters.users.push(el.id)
    //       }
    //     })
    //
    //     console.log(JSON.parse(JSON.stringify(this.filters.users)), 'this.filters.users')
    //   } else {
    //     this.filtersUsers = ''
    //     this.$store.dispatch('browserProfiles/setFilter', { filter: 'users', data: users })
    //     this.$store.dispatch('browserProfiles/loadTags')
    //   }
    //   console.log('filterByUsers - users', users)
    //   // if (users.includes('all')) {
    //   //   users = this.users.all
    //   // }
    //   // users = this.users.all
