...
export default {
  ...
  getters: {
    ...
    synchronizer: state => state.synchronizer,
    browserProfilesLastStartTime: state => state.browserProfilesLastStartTime
  },
  mutations: {
    ...mixinDialogMutations,
    updateField,
    ...
    SET_WEBGL_DATA_LIST: (state, payload) => {
      state.browserProfiles.webglDataList = payload
    },
    ...
    ADD_PENDING_REQUEST: (state, payload) => {
      if (!state.browserProfiles.pendingRequest.includes(payload)) {
        state.browserProfiles.pendingRequest.push(payload)
      }
    },

    REMOVE_PENDING_REQUEST: (state, payload) => {
      state.browserProfiles.pendingRequest = state.browserProfiles.pendingRequest.filter(item => item !== payload)
    },
    ...
    SET_START_TIME: (state, payload) => {
      state.browserProfilesLastStartTime = [...state.browserProfilesLastStartTime, payload]
    },
    ...
    SET_SYNCHRONIZER: (state, payload) => {
      state.synchronizer = payload
    },

    SET_MAC_ADDRESS_VALUE: (state, payload) => {
      state.browserProfile.macAddress.value = payload
    },

    SET_DEVICE_NAME_VALUE: (state, payload) => {
      state.browserProfile.deviceName.value = payload
    }
  },
  actions: {
    ...mixinDialogActions,
    ...
    async loadBrowserProfiles (context, payload = null) {
      ...
      if (context.getters.filters.sortBy.length && context.getters.filters.order.length) {
        if (payload && !payload.sortBy) {
          payload.sortBy = context.getters.filters.sortBy
        }
        if (payload && !payload.order) {
          payload.order = context.getters.filters.order
        }
      }

      if (payload && context.getters.filters) {
        if (!payload.proxies && context.getters.filters.proxies) {
          payload.proxies = context.getters.filters.proxies
        }
        if (!payload.tags && context.getters.filters.tags) {
          payload.tags = context.getters.filters.tags
        }
        if (!payload.mainWebsites && context.getters.filters.mainWebsites) {
          payload.mainWebsites = context.getters.filters.mainWebsites
        }
        if (!payload.statuses && context.getters.filters.statuses) {
          payload.statuses = context.getters.filters.statuses
        }
        if (!payload.users && context.getters.filters.users) {
          payload.users = context.getters.filters.users
        }
      }
      ...
      try {
        ...
        if (response.status === 200 && response.data.data && Array.isArray(response.data.data)) {
          ...
          context.dispatch('sendArchivedProfiles')
          ...
          await context.dispatch('getLocalRunningBrowserProfiles')
          await store.dispatch('proxy/loadProxiesByCurrentBrowserProfilesPage')
        }
      } catch (err) {
        console.error(err)
      } finally {
    ...
    async sendArchivedProfiles ({ state, rootState }) {
      try {
        const browserProfiles = state.browserProfiles.all.map((browserProfile) => browserProfile.id)
        const { changedTariffFromFree, plan } = rootState.main.subscription

        if (changedTariffFromFree && plan && browserProfiles.length > 0) {
          const { data, status } = await this._vm.localApi.post('/upload-data-dir-archive', {
            browserProfiles,
            plan
          })

          if (data.success && status === 200) {
            await this._vm.api.patch('/team/changed-tariff', { changedTariffFromFree: false })
          }
        }
      } catch (err) {
        console.error(err)
    ...
    async getLocalRunningBrowserProfiles (context) {
      try {
        const response = await this._vm.localApi.get('/browser_profiles/running')
        if (response.status === 200 && response.data && typeof response.data === 'object') {
          Object.entries(response.data).forEach(([profileId]) => {
            context.commit('ADD_RUNNING', parseInt(profileId, 10))
          })
        }
      } catch (err) {
        console.error(err)
    ...
    async loadCurrentPage (context) {
      ...
      try {
        ...
        if (context.state.filters.sortBy) {
          request += `sortBy=${context.state.filters.sortBy}&`
        }
        if (context.state.filters.order) {
          request += `order=${context.state.filters.order}&`
        }
        if (request.endsWith('&')) {
          request = request.slice(0, request.length - 1)
        }
        ...
        if (response.status === 200 && response.data.data && Array.isArray(response.data.data)) {
          ...
          data.response.per_page = context.state.pagination.perPage ? context.state.pagination.perPage : data.response.per_page
    ...
    async addBrowserProfile (context, data) {
      try {
        ...
        await context.dispatch('main/getSubscription', null, { root: true })
    ...
    async editBrowserProfile (context, data) {
      ...
      try {
        if (oldBrowserProfileData.notes.content && oldBrowserProfileData.notes.content !== data.data.notes.content) {
          this.amplitudeSendNoteEdit(this.browserProfile, this.subscription)
        }
      ...
      try {
        if (!oldBrowserProfileData.notes.content && data.data.notes.content) {
          this.amplitudeSendNoteAdd(this.browserProfile, this.subscription)
        }
      } catch (_) { console.log(_) }
      ...
      try {
        if (data.data && data.data.proxy && parseInt(oldBrowserProfileData.proxyId) !== parseInt(data.data.proxy.id)) {
          this.amplitudeSendAccountProxyChange(this.browserProfile, this.subscription)
        }
      } catch (_) { console.log(_) }
    ...
    async loadTags (context, payload = { sendAmplitude: false }) {
      try {
        await context.commit('ADD_LOADING', 'tags')
        ...
        if (response.status === 200 && response.data.data && Array.isArray(response.data.data)) {
          ...
          const uniqueTags = []
          tags.forEach((tag) => {
            if (uniqueTags.some((element) => element.name === tag.name)) {
              return
            }
            uniqueTags.push(tag)
          })

          await context.commit('SET_SORTED_TAGS', uniqueTags)
        }
      } catch (err) {
        console.error(err)
      } finally {
        await context.commit('REMOVE_LOADING', 'tags')
      }
    },
    ...
    async getBrowserProfileDeviceName (context, os) {
      try {
        const deviceName = await this._vm.api.get(`/browser_profiles/generate-device-name?os=${os}`)
          .then(res => res.data.deviceName)

        context.commit('SET_DEVICE_NAME_VALUE', deviceName)
      } catch (error) {
        console.log(error.message)
      }
    },

    async getBrowserProfileMacAddress (context) {
      try {
        const response = await this._vm.api.get('/browser_profiles/generate-mac')
        const macAddress = response.data.macAddress

        context.commit('SET_MAC_ADDRESS_VALUE', macAddress)
      } catch (error) {
        console.log(error.message)
      }
    },
    ...
    async deleteBrowserProfiles (context, payload) {
      try {
        const { isFreeTariff } = context.rootState.main
        const url = isFreeTariff ? '/browser_profiles?forceDelete=1' : '/browser_profiles'
        const response = await this._vm.api.delete(url, { data: payload })
    ...
    async setBrowserProfileForEdit (context, browserProfile) {
      const data = {
        ...
        webgpu: {
          mode: browserProfile.webgpu?.mode === 'off' ? 'off' : 'real'
        },
        clientRect: {
        ...
        macAddress: {
          mode: browserProfile.macAddress && typeof browserProfile.macAddress.mode !== 'undefined' ? browserProfile.macAddress.mode : 'off',
          value: browserProfile.macAddress && typeof browserProfile.macAddress.value !== 'undefined' ? browserProfile.macAddress.value : null
        },
        deviceName: {
          mode: browserProfile.deviceName && typeof browserProfile.deviceName.mode !== 'undefined' ? browserProfile.deviceName.mode : 'off',
          value: browserProfile.deviceName && typeof browserProfile.deviceName.value !== 'undefined' ? browserProfile.deviceName.value : null
        },
        screen: {
        ...
        audio: {
          mode: browserProfile.audio && typeof browserProfile.audio.mode !== 'undefined' ? browserProfile.audio.mode : 'real'
        },
        mediaDevices: {
        ...
        args: !Array.isArray(browserProfile.args) ? '' : browserProfile.args.map(arg => '--' + arg).join('\n'),
        ...
        uaFullVersion: browserProfile.uaFullVersion ? browserProfile.uaFullVersion : null,
        login: browserProfile.login ? browserProfile.login : '',
        password: browserProfile.password ? browserProfile.password : '',
        loginpassword: browserProfile.loginpassword ? browserProfile.loginpassword : false
      }
    ...
    async addPendingRequest (context, data) {
      await context.commit('ADD_PENDING_REQUEST', data)
    },

    async removePendingRequest (context, data) {
      await context.commit('REMOVE_PENDING_REQUEST', data)
    },
    ...
    async setStartTime (context, data) {
      await context.commit('SET_START_TIME', data)
    },
    ...
    async setWebglDataList (context, data) {
      context.commit('SET_WEBGL_DATA_LIST', data)
    },
    ...
    async addSynchronizer ({ state, commit }, { session, master, managed, profiles }) {
      const synchronizer = state.synchronizer || {}
      const newSynchronizer = {
        ...synchronizer,
        [session]: { master, managed, started: 0, profiles }
      }

      commit('SET_SYNCHRONIZER', newSynchronizer)
    },

    async removeSynchronizer ({ state, commit }, session) {
      const newSynchronizer = omit(state.synchronizer, session)

      if (isEmpty(newSynchronizer)) {
        commit('SET_SYNCHRONIZER', null)
      } else {
        commit('SET_SYNCHRONIZER', newSynchronizer)
      }
    },

    async removeSynchronizerManaged ({ state, commit }, { session, id }) {
      const newSynchronizer = {
        ...state.synchronizer,
        [session]: {
          ...state.synchronizer[session],
          managed: state.synchronizer[session].managed.filter(i => i !== id)
        }
      }
    ...
    async incrementSynchronizerStarted ({ state, commit }, { session }) {
      const newSynchronizer = {
        ...state.synchronizer,
        [session]: {
          ...state.synchronizer[session],
          started: state.synchronizer[session].started + 1
        }
      }
